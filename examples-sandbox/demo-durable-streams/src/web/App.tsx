...
import { connect, triggerRun } from "./ds.js";
...
export function App() {
    ...
    useEffect(() => {
        ...
        void connect("user", SCOPE).then((d) => {
            dispose = d;
        });
        return () => dispose?.();
    }, []);
    ...
    return (
        <div style={{ maxWidth: 880, margin: "0 auto", padding: "48px 24px" }}>
            <header style={{ marginBottom: 32 }}>
                <h1 style={{ margin: 0, fontSize: 32, letterSpacing: -0.5 }}>livetrace · demo-durable-streams</h1>
                <p style={{ color: "var(--muted)", marginTop: 8, fontSize: 15 }}>
                    Bun + Effect server appends spans to a <a href="https://durablestreams.com" style={{ color: "var(--accent)" }}>durable stream</a> → React reads them back over HTTP → renders here.
                    Reload the tab: history replays from offset 0.
                </p>
            </header>
