 ...
 *
 * Pattern: RAG agent (Plan → Retrieve → Rerank → Generate). The workflow
...
async function handler(req: Request): Promise<Response> {
    ...
    if (url.pathname === "/run" && req.method === "POST") {
        ...
        const query =
            url.searchParams.get("q") ?? "How did Q3 revenue compare to the plan?";

        void Effect.runPromise(
            runWorkflow({ query, scopeId: scope }).pipe(
        ...
        return new Response(JSON.stringify({ ok: true, traceScope: scope, query }), {
...
console.log(`[agent-rag] server http://localhost:${PORT}`);
